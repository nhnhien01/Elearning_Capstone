...
import { Eye, EyeOff, User, Mail, Phone, Lock, Save, Home, Briefcase } from "lucide-react";
...
const StaffProfile = () => {
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      ...
      window.dispatchEvent(new Event("userUpdated"));
      setMsg({ type: "success", text: "Thông tin đã cập nhật!" });
  ...
  if (!profile) return <div className="p-8 text-center text-slate-500 font-black">Đang tải hồ sơ...</div>;
    ...
    <div className="max-w-4xl mx-auto space-y-8 pb-20 mt-8">
      ...
      <div className="flex items-center gap-4 border-b-4 border-slate-900 pb-6">
        ...
        <div className="flex flex-col justify-center h-full">
          <h1 className="text-2xl font-black text-slate-900 uppercase tracking-tighter">Hồ sơ nhân viên</h1>
          <p className="text-slate-500 font-bold text-[10px] uppercase tracking-widest">Consultant Staff</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white p-6 rounded-[2rem] border-[3px] border-slate-900 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] flex flex-col items-center">
          <div className="relative group cursor-pointer mt-4" onClick={() => fileInputRef.current.click()}>
            <img
              ...
              alt="Avatar"
            />
            <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 bg-black/40 rounded-3xl text-white font-black text-[10px] uppercase">Đổi ảnh</div>
          </div>
          ...
          <span className="bg-blue-500 text-white px-4 py-1 rounded-full text-[10px] font-black uppercase mt-4 border-2 border-slate-900 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)]">Nhân Viên</span>
        </div>
        ...
        <div className="md:col-span-2 bg-white p-8 rounded-[2rem] border-[3px] border-slate-900 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)]">
          ...
          <form onSubmit={handleUpdate} className="space-y-5">
            ...
            <div>
              <label className="text-[10px] font-black uppercase text-slate-400 mb-1.5 flex items-center gap-2"><Mail size={12} /> Email</label>
            ...
            <div className="relative">
              <label className="text-[10px] font-black uppercase text-slate-400 mb-1.5 flex items-center gap-2"><Lock size={12} /> Mật khẩu mới</label>
